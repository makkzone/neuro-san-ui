...
import {TEST_AGENT_MATH_GUY, TEST_AGENT_MUSIC_NERD} from "../../../../../__tests__/common/NetworksListMock"
...
const CHAT_WITH_MATH_GUY = `Chat with ${cleanUpAgentName(TEST_AGENT_MATH_GUY)}`
...
describe("ChatCommon", () => {
    ...
    it.each([false, true])("Should render correctly with darkMode=%s", async (darkMode) => {
        ...
        expect(await screen.findByText(cleanUpAgentName(TEST_AGENT_MATH_GUY))).toBeInTheDocument()
        ...
        // Now math guy
        const mathGuyItem = within(connectivityList).queryByText(cleanUpAgentName(TEST_AGENT_MATH_GUY))
    ...
    it("Should handle malformed JSON chunks gracefully", async () => {
        ...
        const agentGreetings = screen.getAllByText(cleanUpAgentName(TEST_AGENT_MATH_GUY))
